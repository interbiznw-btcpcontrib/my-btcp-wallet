...
import zencashjs from 'btcprivatejs'
          ...
          <FormGroup row>
            <Col>
              ...
              <FormText color="muted">
                For Windows, it should be in %APPDATA%/BTCP<br/>
                For Mac/Linux, it should be in ~/.BTCP
              </FormText>
            </Col>
          </FormGroup>
        </Form>
  ...
  render () {
    return (
      <Modal isOpen={this.props.settings.showSettings} toggle={this.props.toggleModalSettings}>
        <ModalHeader toggle={this.props.toggleShowSettings}>BTCP Wallet Settings</ModalHeader>
        <ModalBody>
...
class ZSendZEN extends React.Component {
    ...
    .then(function (tx_resp){
      ...
      .then(function (info_resp){
        ...
        const info_data = info_resp.data
          ...
          // fail and tell user
          if (satoshisSoFar < satoshisToSend + satoshisfeesToSend){
            this.setSendErrorMessage('Not enough confirmed BTCP in account to perform transaction')
            this.setProgressValue(0)
          }
  ...
  render() {
    ...
    if (this.state.sendProgress === 100){
      ...
      zenTxLink = (
        <Alert color="success">
        <strong>BTCP successfully sent!</strong> <a href={zentx} target="_blank">Click here to view your transaction</a>
        </Alert>
      )
    }
    ...
    return (
      <Row>
        <Col>
          <Card>
            <CardBlock>
              <Alert color="danger">ALWAYS VALIDATE YOUR DESINATION ADDRESS BY SENDING SMALL AMOUNTS OF BTCP FIRST</Alert>
              <InputGroup>
              ...
              <FormGroup check>
                <Label check>
                  ...
                  Yes, I would like to send these BTCP
                </Label>
              </FormGroup>
              <br/>
              <Button
...
class ZWalletTabs extends React.Component {
  ...
  savePrivateKeys(){
    ...
    var fileStr = '# Wallet dump created by myBTCPwallet ' + pjson.version + '\n'
    ...
    FileSaver.saveAs(pkBlob, now + '_myBTCPwallet_private_keys.txt')
  }
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      settings: {
        ...
        showWalletGen: false,
        compressPubKey: true,
        insightAPI: 'https://explorer.btcprivate.org/api/',
        explorerURL: 'https://explorer.btcprivate.org/',
        useTestNet: false,
  ...
  render() {
    return (
      <Container>
        <Row>
          <Col>
            <h1 className='display-6'>BTCP Wallet&nbsp;
